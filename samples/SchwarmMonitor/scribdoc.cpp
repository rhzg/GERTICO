...
{
    Federate* pFed = (Federate*)pParam;

	while (true)
	{
		pFed->rtiAmb.tick(0.05, 0.1);
	}

    return 0;   // thread completed successfully
}

void CScribbleDoc::updated()
{
	SetModifiedFlag ();
...
void CScribbleDoc::InitDocument()
{
	...
	// start worker thread for tick
	AfxBeginThread(MyThreadProc, &myFed);
