...
void CScribbleView::OnDraw(CDC* pDC)
{
	...
	// add swarm drawing 
	int w = rectClip.Width();
	int h = rectClip.Height();
	std::map<RTI::ObjectHandle,Fish*>::iterator swarmIt;
	for (swarmIt=pDoc->fedAmb.swarm.begin(); swarmIt!=pDoc->fedAmb.swarm.end();swarmIt++)
	{
		Fish* f = swarmIt->second;
		int xa = f->x.get() * w;
		int ya = f->y.get() * h;
		pDC->Ellipse(xa-2,ya-2,xa+2,ya+2);
	}
	//pDoc->myFed.rtiAmb.tick(0.01, 0.1);
