...
//RTI::FedTime&       myTime= (*(RTI::FedTimeFactory::makeZero()));
...
int main(int argc, char** argv)
{
	...
	// Time Management
	fedAmb.lookahead = 0.1;
	myFed.rtiAmb.enableTimeRegulation (fedAmb.myTime, fedAmb.lookahead);
	myFed.rtiAmb.enableTimeConstrained ();
	while (!fedAmb.timeConstrainedIsEnabled && !fedAmb.timeRegulatedIsEnabled) 
	...
	while (true)
	{
		...
		fedAmb.reqTime = fedAmb.myTime + 0.1;
		myFed.rtiAmb.timeAdvanceRequest (fedAmb.reqTime);
		while (fedAmb.reqTime != fedAmb.myTime)
			myFed.rtiAmb.tick(0.05, 0.5);
	}
